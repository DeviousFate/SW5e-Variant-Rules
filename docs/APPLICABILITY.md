...
- Hunted: Force power casts increase the casting user's Disturbance Point pool by the Force power level. At-wills add 0. Tech powers are ignored. The automation listens to both item-use and dnd5e activity-use workflows so combat sheet/chat-card power use is captured. The log records detected class, feat, and other Force-power sources from the actor's owned items.
