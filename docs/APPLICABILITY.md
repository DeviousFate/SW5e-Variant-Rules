...
- Hunted: Force power casts during combat encounters increase the casting user's active Disturbance Point pool by the Force power level. At-wills add 0. Tech powers are ignored. At combat end, the GM rolls d100 against each active pool; rolling equal to or below the pool records a Hunted status and advances the hunter perception tag. The automation listens to both item-use and dnd5e activity-use workflows so combat sheet/chat-card power use is captured.
